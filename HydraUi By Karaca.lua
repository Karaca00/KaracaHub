...
function UILibrary.new(gameName, userId, rank)
    ...
    GUI.Name = "Karaca Hub"
    if game:GetService("CoreGui"):FindFirstChild(GUI.Name) or LocalPlayer:WaitForChild("PlayerGui"):FindFirstChild(GUI.Name) then
        if RunService:IsStudio() == false then
            game:GetService("CoreGui"):FindFirstChild(GUI.Name):Destroy()
        else
            LocalPlayer:WaitForChild("PlayerGui"):FindFirstChild(GUI.Name):Destroy()
        end
    end
    GUI.Parent =
